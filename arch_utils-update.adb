...
separate (Arch_Utils)
procedure Update (Arch_Name : in String; File : in String_List; Msg : out String_List; Directory : in String := "") is
...
begin -- Update
   ...
   if Ada.Directories.Exists (Arch_Name) then
      ...
      Copy_Unchanged : loop
         ...
         begin -- Check_One
            if Name.Contains (Header.Name) then -- To be updated
               Skip (File => Curr, Count => Header.Compressed_Length);
               Deleted.Append (New_Item => Header.Name);
            else -- Unchanged
   ...
   New_Files : for I in 1 .. Name.Last_Index loop
      Make_Header : declare
         File_Name : String renames Name.Element (I);

         Dir_Name : constant String := Directory & File_Name;

         Header     : Header_Info (Name_Length => File_Name'Length);
         Compressed : Byte_List;
      begin -- Make_Header
         if not Ada.Directories.Exists (Dir_Name) then
            if not Deleted.Contains (File_Name) then
               Msg.Append (New_Item => "File " & Dir_Name & " does not exist; ignoring");
            else
               Msg.Append
                  (New_Item => "File " & File_Name & " found in " & Arch_Name & " but replacement does not exist; aborting");
